...
import ru.otus.otuskotlin.yan.swiftorder.mappers.toResponseObject
...
fun Context.toTransportCreate(): OrderCreateResponse = OrderCreateResponse(
    ...
    order  = orderResponse.toResponseObject().takeIf { orderResponse.id.asString.isNotBlank() },
)
...
fun Context.toTransportSearch(): OrderSearchResponse = OrderSearchResponse(
    result = state.toResult(),
    errors = errors.toTransportErrors(),
    orders = ordersResponse.map { it.toResponseObject() }.takeIf { it.isNotEmpty() },
)

// ---- State / Error helpers ----

fun SOContextState.toResult(): ResponseResult? = when (this) {
    SOContextState.RUNNING   -> ResponseResult.SUCCESS
    SOContextState.FINISHING -> ResponseResult.SUCCESS
    SOContextState.FAILING   -> ResponseResult.ERROR
    SOContextState.NONE      -> null
}
